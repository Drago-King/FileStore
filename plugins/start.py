...
def clean_caption(caption):
    """
    Remove all embedded links and the line '⚜️ Powered By : [ @BoB_Files1 ]' from the caption.
    """
    if not caption:
        return caption
    # Remove all markdown and HTML links
    # Remove [text](url) and <a href="url">text</a>
    caption = re.sub(r'\[([^\]]+)\]\(([^)]+)\)', r'\1', caption)
    caption = re.sub(r'<a\s+href=["\"][^"\"]+["\"][^>]*>(.*?)<\/a>', r'\1', caption)
    # Remove plain URLs
    caption = re.sub(r'https?://\S+', '', caption)
    # Remove the powered by line (with or without spaces)
    caption = re.sub(r'^.*⚜️\s*Powered\s*By\s*:\s*\[\s*@BoB_Files1\s*\].*$', '', caption, flags=re.MULTILINE)
